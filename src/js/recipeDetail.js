...
var vm = new Vue({
    ...
    methods: {
        ...
        isCollected:function(){
            this.$http.get(myJS.url('recipes/'+recipeId+'/isCollected'))
                .then((response) => {
                    response = response.body;
                    if(response.errcode === 0){
        ...
        doCollectOrCancel:function(){
            if(this.isCollectedFlag){
                this.$http.post(myJS.url('recipes/'+recipeId+'/unCollect'))
                .then((response) => {
                    response = response.body;
                    if(response.errcode === 0){
                        //alert('取消成功'+e.target);
                ...
                this.$http.post(myJS.url('recipes/'+recipeId+'/collect'))
                .then((response) => {
                    response = response.body;
                    if(response.errcode === 0){
                        //alert('收藏成功'+e.target);
        ...
        getRecipeDetail:function(){
            this.$http.get(myJS.url('recipes/'+recipeId))
                .then((response) => {
